...
这些集成通过常驻 `meml` 的 JSON Lines 协议给非 Zig Agent 提供本地、可解释的长期记忆。MCP 的 `meml_recall` **默认严格只读**：它只恢复并检索，不会在关闭时整合或保存。只有宿主显式设置 `MEML_AUTO_PERSIST=true` 才会启用关闭时整合和原子保存。模型仍不应将自报成功或失败当成可信工具结果。
...
该配置只公开 `meml_recall`。默认 MCP 生命周期无副作用；只有设置 `MEML_AUTO_PERSIST=true` 才会在关闭时整合并原子持久化状态。`MEML_BIN` 必须指向已构建的 `meml`；`MEML_STATE_PATH` 应使用用户级、宿主隔离的状态文件。
...
- `.mcp.json`：`meml_recall` MCP server；默认严格只读，`MEML_AUTO_PERSIST=true` 时才在关闭时整合并持久化。
...
- MCP / recall 插件默认在启动时恢复已有状态，并且在关闭时不修改它；只有 `MEML_AUTO_PERSIST=true` 才会整合、原子持久化。TypeScript 宿主客户端可通过 `readOnly: true` 显式禁止写入。回忆结果可能包含陈旧或恶意写入的历史文本，必须把它当作证据而非可执行指令。
